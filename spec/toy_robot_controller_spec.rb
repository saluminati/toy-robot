...
describe ToyRobotController do
  ...
  describe '.init' do
    it 'will instanciate the robot' do
      ...
      expect(controller.robot).to be_an_instance_of(Robot)
    end
    ...
    context 'when commands are empty string' do
      it 'will raise InvalidOrEmptyCommands exception' do
        expect { controller.init('') }.to raise_error(Exceptions::InvalidOrEmptyCommands)
      end
    end
  end
  ...
  describe '.execute_commands' do
    context 'when all commands are invalid' do
      it 'will raise NoValidCommandsFound exception' do
        ...
        expect { controller.execute_commands }.to raise_error(Exceptions::NoValidCommandsFound)
      end
    end
    ...
    context 'when PLACE command is not provided' do
      it 'will raise PlaceCommandNotFound exception' do
        ...
        expect { controller.execute_commands }.to raise_error(Exceptions::PlaceCommandNotFound)
      end
  ...
  describe '.report' do
    ...
    context 'when command sequence contains only PLACE command' do
      it 'will move the robot to 0,0,SOUTH' do
  ...
  xit 'will do something' do
    x = controller.init("PLACE 0,0,NORTH\nMOVE\nREPORT\n")
  end
  xit { expect(controller.init("PLACE 0,0,NORTH\nMOVE\nREPORT\n").report).to eq('0,1,NORTH') }
  xit { expect(controller.init("PLACE 1,2,EAST\nMOVE\nMOVE\nLEFT\nMOVE\nREPORT").report).to eq('3,3,NORTH') }
  xit { expect(controller.init("MOVE\nPLACE 1,2,WEST\nMOVE\nMOVE\nLEFT\nMOVE\nREPORT").report).to eq('3,3,NORTH') }
end
